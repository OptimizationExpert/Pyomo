...
def opt_path(direction: int, minlim: int, maxlim: int):
    ...
    arcs = [(i, j, v) for (i, j), v in x.items()] + [(j, 1, v_link) for j, v_link in fn.items()] + [(i, i, v.Not()) for
                                                                                                    i, v
                                                                                                    in select.items()]
    ...
    out = {(i, j) for (i, j), v in x.items() if solver.value(v) > 0}
    return out, int(solver.best_objective_bound)


xp, ofmax = opt_path(1, 0, 20000)
xn, ofmin = opt_path(-1, 0, ofmax)
xr, of = opt_path(0, ofmin, ofmax)
