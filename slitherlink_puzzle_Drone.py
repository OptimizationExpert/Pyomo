...
U = {(i.name, j.name, dr): model.NewBoolVar(f"connection_{i.name}_{j.name}_{dr}") for i in nodes_all for j in nodes_all
     for dr in drones if
     neighbour(i, j)}
selected = {(i.name, dr): model.NewBoolVar(f"select_{i.name}_{dr}") for i in nodes_all for dr in drones}
drone_used = {dr: model.NewBoolVar(f"drone_{dr}") for dr in drones}
ctodr = {(cell.name, dr): model.NewBoolVar(f"cell_{cell.name}_drone_{dr}") for cell in cells_all for dr in drones if
...
for dr in drones:
    ...
    # model.add( cp_model.LinearExpr.sum(travelled_arcs) <= 20)
